...
async def _memory_job_status(job_id: str, poll_url: Optional[str] = None) -> dict[str, Any]:
    ...
    # Successful job envelopes include ``error: null`` and failed jobs include
    # a useful terminal ``error`` string. Only an HTTP/client error envelope is
    # already normalized; every actual job payload must pass through the job
    # shaper so callers consistently receive ``memories``/``searchable`` fields.
...
async def hebbrix_extraction_status(
    ...
    if not job_id or len(job_id) > 64:
        return {"error": "job_id must contain 1 to 64 characters", "ok": False}
